...
def start(producer,pid,render_id,assigned_to,*,actor,key,mode='full'):
    require(actor=='producer','PRODUCER_REQUIRED')
    require(mode in ('full','lyrics'),'INVALID_ANALYSIS_MODE')
    require(assigned_to in ('yinyue','xiaowu'),'UNKNOWN_AGENT')
    if mode=='full':require(producer.bridge and assigned_to in producer.bridge.bindings,'HERMES_UNAVAILABLE')
    ...
    def create(db):
        ...
        if mode=='full':existing=[c for c in existing if c.get('analysis_mode','full')=='full']
        if mode=='lyrics':
            completed=[c for c in existing if c['state']=='completed' and c.get('asr_sha256')]
            if completed:return completed[-1]
            existing=[c for c in existing if c.get('analysis_mode')=='lyrics']
        ...
        c=dict(schema_version='producer-command/4',command_id=new_id('command'),project_id=pid,
            ...
            source_asset=asset,source_duration=job['verification']['duration_seconds'],kind='analyze',assigned_to=assigned_to,analysis_mode=mode,
    ...
    request=dict(op='audio_analysis_start',project_id=pid,render_id=render_id,assigned_to=assigned_to)
    if mode!='full':request['mode']=mode
    c=producer.store._operation(actor,key,request,create)
    ...
    with producer.store.connect() as db:
        ...
        if c['state']=='preparing':
            c['state']='analyzing' if c.get('analysis_mode')=='lyrics' else 'queued'
            producer._save(db,c,actor)
        db.commit()
    if c.get('analysis_mode')=='lyrics' and c['state']=='analyzing':resume(producer,pid,c['command_id'])
    return c
...
def packet(producer,pid,cid,*,actor):
    ...
    require(command.get('analysis_mode')!='lyrics','LOCAL_ANALYSIS_ONLY')
...
def resume(producer,pid,cid):
    ...
    require(c.get('analysis_dispatched') or c.get('analysis_mode')=='lyrics','ANALYSIS_NOT_DISPATCHED')
    with ACTIVE_LOCK:
...
def read(producer,pid,render_id):
    ...
    # A newer transcription request must not hide an already completed lyric
    # analysis for this exact audio while it waits for SheetSage.
    if not result.get('asr'):
        completed=[v for v in producer.list(pid) if v['kind']=='analyze' and
                   v['source_render_id']==render_id and v['state']=='completed' and v.get('asr_sha256')]
        if completed:
            prior=completed[-1];digest=prior['asr_sha256']
            with producer.store.connect() as db:raw=db.execute('SELECT content FROM blobs WHERE hash=?',(digest,)).fetchone()[0]
            require(sha(raw)==digest,'ANALYSIS_BLOB_MISMATCH')
            result['asr']=json.loads(raw);result['asr_command_id']=prior['command_id']
    ...
    result['stage']='completed' if c['state']=='completed' else 'recognizing_lyrics' if result.get('transcription') or c.get('analysis_mode')=='lyrics' else 'waiting_transcription'
    return result
